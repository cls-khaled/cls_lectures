...
imageContainer.addEventListener(events[deviceType].move, (e) => {
  ...
  try {
    //pageX and pageY return the position of client's cursor from top left  screen
  ...
  if (
    imageWidth - (x - imageContainer.offsetLeft) < 100 ||
    x - imageContainer.offsetLeft < 100 ||
    imageHeight - (y - imageContainer.offsetTop) < 100 ||
    y - imageContainer.offsetTop < 100
  ) {
    hideElement();
  } else {
